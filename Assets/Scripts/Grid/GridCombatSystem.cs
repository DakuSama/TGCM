...
public class GridCombatSystem : MonoBehaviour
{
    ...
    {
        order = true;
        index = GameManager.saveIndexCurrentEnemy;
        maxTurn = 20;
    ...
    public void SetNotWalkableEnemy()
    {
        foreach(UnitGridCombat unitGridCombatTest in enemyTeamList)
        {
            GameHandler_GridCombatSystem.Instance.GetGrid().GetXY(unitGridCombatTest.GetPosition(), out int gridX, out int gridY);
            GameHandler_GridCombatSystem.Instance.pathfinding.GetNode(gridX, gridY).SetIsWalkable(false);
        }
    }

    public void SetWalkableEnemy()
    {
        foreach (UnitGridCombat unitGridCombatTest in enemyTeamList)
        {
            GameHandler_GridCombatSystem.Instance.GetGrid().GetXY(unitGridCombatTest.GetPosition(), out int gridX, out int gridY);
    ...
    public void ConditionDefeat()
    {
        if (allieTeamList.Count <= 0 || currentTurn >= maxTurn)
        {
            ...
            MusicSettings.MSInstance.PauseMusic();
            lose.Play();
        }
    }
    ...
    public void TrunBase()
    {
        ...
        if (whoPlay == WhoPlay.Enemy)
        {
            if (_turnBased > 1)
            {
                foreach (UnitGridCombat unitGridCombatTest in enemyTeamList)
                {
                    foreach (int test in indexlist)
                    {
                        if (unitGridCombatTest.Index == test)
                        {
                            break;
                        }
                        else
                    ...
                    if (testindex == indexlist.Count)
                    {
                        if (unitGridCombatTest.FinishThisTurn())
                        {
                            scorefinish++;
                            order = true;
                            indexlist.Insert(unitGridCombatTest.Index, unitGridCombatTest.Index);
                        }
                    }
                    testindex = 0;
                }
                if (scorefinish == enemyTeamList.Count)
                {
                    scorefinish = 0;
                    indexlist.Clear();
                ...
                if(enemyTeamList[x].FinishThisTurn())
                {
                    if(!(x == enemyTeamList.Count - 1))
                    {
                        if (order)
                        {
                            order = false;
                            ...
                            x++;
                            GameManager.saveIndexCurrentEnemy = GridCombatSystem.Instance.index;
                        }
                    }
                    else
                    {
